  ...
  static toDomain(typeorm: BookTypeormEntity): Book {
    return Book.create({
      authors: typeorm.authors
        ? typeorm.authors.map((typeormAuthor) =>
            AuthorMapper.toDomain(typeormAuthor),
          )
        : [],
      availableCopyCount: typeorm.availableCopyCount,
      category: BookCategory.create(
        parseEnum(BookCategoryEnum, typeorm.category),
      ),
      code: BookCode.create(typeorm.code),
      copies: typeorm.copies
        ? typeorm.copies.map((typeormCopy) =>
            BookCopyMapper.toDomain(typeormCopy),
          )
        : [],
      description: typeorm.description,
      lastCopyNo: typeorm.lastCopyNo,
